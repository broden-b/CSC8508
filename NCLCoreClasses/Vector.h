...
namespace NCL::Maths {
	...
	namespace Vector {
		...
		template <typename T>
		constexpr VectorTemplate<T, 2> Transpose(const VectorTemplate<T, 2>& v) {
			return VectorTemplate<T, 2>(v.x, v.y); // This could represent a matrix transpose, but it's essentially the same for a 2D vector.
		}

		// Transpose for 3D vector (as a matrix)
		template <typename T>
		constexpr VectorTemplate<T, 3> Transpose(const VectorTemplate<T, 3>& v) {
			return VectorTemplate<T, 3>(v.x, v.y, v.z); // Similar concept, treating this as a 1x3 or 3x1 matrix.
		}

		// Transpose for 4D vector (as a matrix)
		template <typename T>
		constexpr VectorTemplate<T, 4> Transpose(const VectorTemplate<T, 4>& v) {
			return VectorTemplate<T, 4>(v.x, v.y, v.z, v.w); // Treating as a matrix, this is conceptually the transpose of a 1x4 or 4x1 matrix.
		}
	}
}