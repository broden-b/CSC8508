...
void PositionConstraint::UpdateConstraint(float dt) {
	...
	if (abs(offset) > 0.0f)
	{
		...
		PhysicsObject* physicsA = objectA->GetPhysicsObject();
		PhysicsObject* physicsB = objectB->GetPhysicsObject();

		Vector3 relativeVelocity = physicsA->GetLinearVelocity() - physicsB->GetLinearVelocity();

		float constraintMass = physicsA->GetInverseMass() + physicsB->GetInverseMass();
		...
		if (constraintMass > 0.0f) {
			...
			physicsA->ApplyLinearImpulse(aImpulse);
			physicsB->ApplyLinearImpulse(bImpulse);
		}
	}
}